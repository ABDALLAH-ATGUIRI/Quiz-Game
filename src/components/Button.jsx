...
export const Button = ({ title, handle, index, correctAnswer }) => {
  ...
  return (
    ...
    >
      {isSelect === true && (
        <span
          className="text-white text-sm font-bold rounded-full w-6 h-6 bg-black/10 mr-8 border-2 border-white"
          style={{
      ...
      {isSelect === null && (
        <span className="text-black text-sm font-bold rounded-full w-6 h-6 bg-white mr-8 border-2 border-white">
          {index + 1}
        </span>
      )}
      {title}
